...
function build_page()
{
    ...
    while ($row=mysql_fetch_assoc($res))
    {
        if( $row[ 'cnt' ] > 0 ) {
            ...
            if ( $dfe_old!=$row['dt'].$row['form'].$row['exam'] & $dfe_old!='' )
            {
                $stages .= str_replace(
                    array('%stages_new%','%dt%','%form%','%exam%'),
                    array($stages1, $row['dt'], $row['form'], $row['exam']),
                    $tpl['stages']['stage_line1']);
                $dfe_old = $row['dt'].$row['form'].$row['exam'];
            }
...
//стадия 0=призеры прошлого года,1=отборочный этап и тд
$tpl['stages']['stages_list']=
    '%error%
<!--<style>
...
<style>
    ...
    .ExpandOpen .Expand {
        background-image: url(www/img/imgTree/minus.gif);
    }

    /* closed is higher priority than open */
    .ExpandClosed .Expand {
        background-image: url(www/img/imgTree/plus.gif);
    }

    /* highest priority */
    .ExpandLeaf .Expand {
        background-image: url(www/img/imgTree/leaf.gif);
    }
    ...
    * html  .Content {
    ...
    .ExpandLoading   {
        ...
        height: 18px;
        float: left;
        background-image: url(www/img/imgTree/loading.gif);
    }
...
//строка в каждой стадии
$tpl['stages']['stage_line2']=
    '<li class="Node ExpandLeaf IsLast">
             <span class="Content">%place%</span>
             <span class="Content">%school_class%</span>
      </li>';
...
<div onclick="tree_toggle(arguments[0])">
    <ul class="Container">
         ...
         <span class="Content">%dt%</span>
         <span class="Content">%exam%</span>
         <span class="Content">%form%</span>
             <ul class="Container">
                 %stages_new%
             </ul>
         </li>
